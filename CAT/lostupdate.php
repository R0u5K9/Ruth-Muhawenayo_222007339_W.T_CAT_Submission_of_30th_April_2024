...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
   ...
   $sql = "SELECT * FROM lost_items WHERE item_id=$Iid";
   ...
   if (!$row) {
    ...
    do {
       ...
       if  ( empty($Iid) || empty($Iname) || empty($desc) ||  empty($locat) ||  empty($ldate) ) {
            $errorMessage = "All fields are required";
            break;
       }

       $sql = "UPDATE lost_items SET item_name='$Iname', description='$desc', location='$locat', lost_date='$ldate' WHERE item_id= $Iid";
    $result = $connection->query($sql);
...
<form method="post">
    ...
    <div class="row mb-3">
        ...
        <a class='btn btn-outline-primary btn-sm' href="lost.php" role="button">cancel</a>
      </div>
        </div>
